# CLAUDE.md — Workflow Analytics Plugin
...
```sh
GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -ldflags="-s -w" -o workflow-plugin-analytics ./cmd/workflow-plugin-analytics/
```

## Structure

- `cmd/workflow-plugin-analytics/main.go` — Plugin entry point (calls `sdk.ServePluginFull`)
